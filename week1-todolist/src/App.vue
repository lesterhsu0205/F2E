    ...
    <div class="container my-4">
      ...
      <div class="mt-4">
        <transition name="slide">
          <edit-form v-if="isNewTodo && !currentEdit.id" :todo="currentEdit" @refresh="getData" @closeEdit="closeEdit"></edit-form>
        </transition>
        ...
        <draggable v-model="todoItem" :options="{handle: '.handle'}" @end="sort">
          <div v-for="item in todoItem" :key="item.id" v-if="currentTab === 'myTask' || currentTab == item.completed">
            <item v-if="!Object.is(currentEdit.id, item.id)" :todo="item" @refresh="getData" @edit="edit"></item>
            <edit-form v-if="Object.is(currentEdit.id, item.id)" :todo="currentEdit" @refresh="getData" @closeEdit="closeEdit"></edit-form>
          </div>
        </draggable>
      </div>
      <!-- outer -->
